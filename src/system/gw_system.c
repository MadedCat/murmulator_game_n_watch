...
static void (*device_blit)(uint8_t *active_framebuffer);
...
void gw_system_blit(uint8_t *active_framebuffer) { device_blit(active_framebuffer); }
bool gw_system_romload() { return gw_romloader(); }

/******** Audio functions *******************/
volatile uint8_t mspeaker_data = 0;
...
unsigned char gw_readK(unsigned char io_S)
{
	...
	//const unsigned int KEY_SOFT_GAME_A = GW_BUTTON_A + GW_BUTTON_GAME;
	//const unsigned int KEY_SOFT_GAME_B = GW_BUTTON_B + GW_BUTTON_GAME;

	if ((gw_buttons >> 10) & 1)
	{
		keys_pressed = 0;
		key_soft_value = KEY_SOFT_TIME;
		//printf("KEY_SOFT_TIME\n");
	}
	if ((gw_buttons >> 11) & 1)
	{
		keys_pressed = 0;
		key_soft_value = KEY_SOFT_ALARM;
		//printf("KEY_SOFT_ALARM\n");
	}
	...
	if ((keys_pressed == 0) & (key_soft_value == 0)) return 0;
			...
			else
			{
				...
				{	
					...
					if (((gw_keyboard[Sx] & GW_MASK_K4) == (key_soft_value << 24)))
						io_K |= 0x8;
					//printf("check soft keys:[%08X][%08X][%02X][%02X]\n",key_soft_value,gw_keyboard[Sx],Sx,io_K);
				}
				// check hardware keys excluding soft keys
				else
				{	//printf("check hardware keys excluding soft keys:[%04X]\n",keys_pressed);
					...
					if (((gw_keyboard[Sx] & GW_MASK_K4) & (keys_pressed << 24)) != 0)
							...
							io_K |= 0x8;
					//printf("check hard keys:[%08X][%08X][%02X][%02X][%02X]\n",keys_pressed,gw_keyboard[Sx],Sx,io_K,io_S);
				}
			}
		}
		//case of R/S output is not used to poll buttons (used R2 or S2 configuration)
...
/* 		if (io_S == 0)
	...
	//if(io_K>0) printf("io_S:[%02X]  io_K:[%02X]\n",io_S,io_K);
	return io_K & 0xf;
}

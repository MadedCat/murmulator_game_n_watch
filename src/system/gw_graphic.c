...
static uint8_t *gw_graphic_framebuffer = 0;

static uint8_t *source_mixer = 0;
...
static inline uint8_t rgb_multiply_8bits(uint8_t bg, uint8_t sg){

	//return (uint8_t)((bg+(PALETTE_IDX_INC*2))&0xFF);
	
	//printf("sg:[%02X]\n",sg);
	//return (uint8_t)((bg+(PALETTE_IDX_INC*sg))&0xFF);
	...
	//return (uint8_t)(bg);
	/*
	if(sg==3) return (uint8_t)((bg+(PALETTE_IDX_INC*3))&0xFF);
	if(sg==2) return (uint8_t)((bg+(PALETTE_IDX_INC*2))&0xFF);
	if(sg==1) return (uint8_t)((bg+(PALETTE_IDX_INC*1))&0xFF);
	if(sg==0) return (uint8_t)((bg+(PALETTE_IDX_INC*0))&0xFF);
	*/

	//return (uint8_t)(bg&0x3F);

	//if(sg==SEG_TRANSPARENT_COLOR) return (uint8_t)(bg&0xFF);
	//printf("sg:[%02X]\n",sg);
	//if(sg==3) return (uint8_t)((bg+(PALETTE_IDX_INC*2))&0xFF);
...
__attribute__((optimize("unroll-loops"))) static inline void update_segment_4bits(uint8 segment_nb, bool segment_state){
	...
	for (int line = segments_y; line < segments_height + segments_y; line++)
	{
		for (int x = segments_x; x < segments_width + segments_x; x++)
		{
			...
			if (segment_state){
				if (cur_pixel != SEG_TRANSPARENT_COLOR){
					gw_graphic_framebuffer[line * GW_SCREEN_WIDTH + x] =  rgb_multiply_8bits(source_mixer[line * GW_SCREEN_WIDTH + x], (cur_pixel&0x04));
				}/* else {
					gw_graphic_framebuffer[line * GW_SCREEN_WIDTH + x] =  source_mixer[line * GW_SCREEN_WIDTH + x];	
				}*/
			} else {
				if (cur_pixel != SEG_TRANSPARENT_COLOR){
					//gw_graphic_framebuffer[line * GW_SCREEN_WIDTH + x] =  source_mixer[line * GW_SCREEN_WIDTH + x];
					if (gw_head.flags & FLAG_RENDERING_LCD_INVERTED){
						gw_graphic_framebuffer[line * GW_SCREEN_WIDTH + x] =  (((source_mixer[line * GW_SCREEN_WIDTH + x])+(PALETTE_IDX_INC*3))&0xFF);
					} else {
						gw_graphic_framebuffer[line * GW_SCREEN_WIDTH + x] =  (((source_mixer[line * GW_SCREEN_WIDTH + x])+(PALETTE_IDX_INC*1))&0xFF);
					}
				}
			}
...
__attribute__((optimize("unroll-loops"))) inline void gw_gfx_sm510_rendering(uint8_t *framebuffer)
{
	/*
...
*/
	...
	if (gw_head.flags & FLAG_RENDERING_LCD_INVERTED)
	{
		...
		//memset(framebuffer, 0, GW_SCREEN_WIDTH * GW_SCREEN_HEIGHT);
		SEG_TRANSPARENT_COLOR = SEG_BLACK_COLOR;
		source_mixer = (uint8_t*)gw_background;
	}
	else
	{
...
__attribute__((optimize("unroll-loops"))) inline void gw_gfx_sm500_rendering(uint8_t *framebuffer)
{
	/*
...
*/
	...
	{
		//memcpy(framebuffer, gw_background, GW_SCREEN_WIDTH * GW_SCREEN_HEIGHT);
		SEG_TRANSPARENT_COLOR = SEG_WHITE_COLOR;
		source_mixer = (uint8_t*)gw_background;//framebuffer;
	}
	//memcpy(framebuffer, gw_background, GW_SCREEN_WIDTH * GW_SCREEN_HEIGHT);
	// 2 columns z
	for (int h = 0; h < 2; h++)
